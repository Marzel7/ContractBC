...
const snapshot = async (compoundERC20, wbtc, c_wbtc) => {
  const {exchangeRate, supplyRate} = await compoundERC20.callStatic.getInfo();
...
describe("CompoundERC20 deployment", function () {
  it("confirm balances", async function () {
    expect(await getBalance(compoundERC20.address)).to.eq(toWei(100));
    expect(await wbtc.balanceOf(c_wbtc.address)).to.be.gt(0);
  });

  it("should supply and redeem", async () => {
    let whaleBalance = fromWei8(8, await wbtc.balanceOf(WHALE));
    let compoundWBTCBalance = fromWei8(8, await wbtc.balanceOf(compoundERC20.address));
    ...
    let compoundWBTCBalanceAfter = fromWei8(8, await wbtc.balanceOf(compoundERC20.address));
